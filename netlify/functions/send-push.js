...
exports.handler = async function(event) {
  ...
  // This endpoint is public (any guest's browser can reach it) - the per-subscription diagnostic
  // detail (DB ids, partial endpoint URLs) added tonight for live debugging must never go out on
  // that public response, only to an authenticated admin caller (e.g. a manual curl test with the
  // admin token). Everyone else just gets a bare ok.
  return { statusCode: 200, body: JSON.stringify(requireAdmin(event) ? { ok: true, ...result } : { ok: true }) };
};
